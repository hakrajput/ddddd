...
def sort_date_cols(cols: list[str]) -> list[str]:
    def parse(c: str) -> datetime.datetime:
        try:
            year = datetime.date.today().year
            return datetime.datetime.strptime(f"{c}-{year}", "%d-%B-%Y")
        except ValueError:
...
def read_and_prepare(
    ...
    date_cols_sorted = sort_date_cols(date_cols)
...
def generate_station_summary(writer: pd.ExcelWriter, df: pd.DataFrame) -> None:
    ...
    power_status_format = workbook.add_format({"font_size": 9, "bold": True, "bg_color": "#CCD4BE"})
    current_row = 0
    for power in working["Power Status"].dropna().unique():
        ...
        worksheet.write(current_row, 0, "Power Status", power_status_format)
        worksheet.write(current_row, 1, power, power_status_format)
        current_row += 1
...
def generate_tch_vs_nar(writer: pd.ExcelWriter, tch_df: pd.DataFrame, nar_df: pd.DataFrame) -> pd.DataFrame:
    ...
    common_dates = sort_date_cols(list(set(date_cols_tch) & set(date_cols_nar)))
    if not common_dates:
...
def generate_belt_overview(writer, tch_df, fuel_df=None, tchvsnar_df=None):
    ...
    date_cols = [c for c in working.columns if "-" in str(c) and str(c).split("-")[0].isdigit()]
    date_cols_sorted = sort_date_cols(date_cols)
...
def generate_2g_4g_delta(writer, tch_df, lte_path):
    ...
    if lte_numeric_headers:
        ...
        lte_df = tmp
        lte_candidates = sort_date_cols(get_month_date_cols(lte_df))

    # TCH candidates (already dd-Month strings)
    tch_candidates = sort_date_cols(get_month_date_cols(tch_df))
...
def generate_degradation_analysis(writer, df, outage_path, sheet_name="Degradation Analysis"):
    ...
    if benchmark_date == latest_date:
        result.rename(
            columns={latest_date: "Current Availability", "Category": "Outage Category"},
            inplace=True,
        )
        result["Benchmark Availability"] = result["Current Availability"]
    else:
        result.rename(
            columns={
